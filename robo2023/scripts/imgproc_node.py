...
class ImgNode(Node) : 
    ...
    def imgproc(self):
        ...
        output = self.conclusion_output(store_sample)
        print(output)
        cap.release()
        return output
    ...
    def sort_order(self,first_row, second_row,third_row):
        index = [self.text_check(e) for e in third_row]
    ...
    def classify_shape(self,image_list):
    ...
    def generate_output(self,frame):
        store_shape = []
        store_numbers = []
        store_binary = []
        image = self.find_largest_rectangle(frame)
        found_numbers = []
        message = ""
        # scale_factor = 2
        ...
        first_row, second_row, third_row = self.segment_image(image)
        for i in range(5):
        ...
        for i in range(5):
            ...
            if i != 4 :
                message += ','
        return message
    def conclusion_output(self,samples):
        ...
        while -1 in arealist :
            arealist.remove(-1)
        shape = self.classify_shape(arealist)
        for i in range(5):
